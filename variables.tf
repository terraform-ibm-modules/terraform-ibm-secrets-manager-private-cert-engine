...
variable "certificate_templates" {
  type = list(object({
    name                               = string                                                                          # Name of the Certificate Template to create for a private_cert secret engine
    max_ttl                            = optional(string, "8760h")                                                       # Max TTL for the certificate template
    allow_any_name                     = optional(bool, true)                                                            # Allow clients to request a private certificate that matches any common name
    allow_bare_domains                 = optional(bool, false)                                                           # Allow clients to request private certificates that match the value of the actual domains on the final certificate
    allow_glob_domains                 = optional(bool, false)                                                           # Allow glob patterns in the names that are specified in the allowed_domains field
    allow_ip_sans                      = optional(bool, true)                                                            # Allow clients to request a private certificate with IP Subject Alternative Names
    allow_subdomains                   = optional(bool, false)                                                           # Allow clients to request private certificates with common names (CN) that are subdomains of the CNs that are allowed by the other certificate template options
    allowed_domains                    = optional(list(string), [])                                                      # Domains to define for the certificate template
    allowed_domains_template           = optional(bool, false)                                                           # Allow the domains that are supplied in the allowed_domains field to contain access control list (ACL) templates
    allowed_other_sans                 = optional(list(string), [])                                                      # The custom Object Identifier (OID) or UTF8-string Subject Alternative Names (SANs) to allow for private certificates
    allowed_secret_groups              = optional(string, null)                                                          # Allowed secrets group Ids as a comma-delimited list
    allowed_uri_sans                   = optional(list(string), ["example.com/test"])                                    # Allowed URI SANs for the certificate template
    basic_constraints_valid_for_non_ca = optional(bool, false)                                                           # Mark the Basic Constraints extension of an issued private certificate as valid for non-CA certificates
    client_flag                        = optional(bool, true)                                                            # Set whether private certificates are flagged for client use
    code_signing_flag                  = optional(bool, false)                                                           # Set whether private certificates are flagged for code signing use
    email_protection_flag              = optional(bool, false)                                                           # Set whether private certificates are flagged for email protection use
    enforce_hostnames                  = optional(bool, true)                                                            # Set whether to enforce only valid host names for common names, DNS Subject Alternative Names, and the host section of email addresses
    ext_key_usage                      = optional(list(string), [])                                                      # List of allowed extended key usage constraint on private certificates
    ext_key_usage_oids                 = optional(list(string), [])                                                      # List of extended key usage Object Identifiers (OIDs)
    key_usage                          = optional(list(string), ["DigitalSignature", "KeyAgreement", "KeyEncipherment"]) # List of allowed key usage constraint to define for private certificates
    policy_identifiers                 = optional(list(string), [])                                                      # List of policy Object Identifiers (OIDs)
    require_common_name                = optional(bool, true)                                                            # Set whether to require a common name to create a private certificate
    server_flag                        = optional(bool, true)                                                            # Set whether private certificates are flagged for server use
    serial_number                      = optional(string, null)                                                          # Serial number to assign to the generated certificate
    use_csr_cn                         = optional(bool, true)                                                            # Set whether to use the common name (CN) from a certificate signing request (CSR) instead of the CN that's included in the data of the certificate
    use_csr_sans                       = optional(bool, true)                                                            # Set whether to use the Subject Alternative Names(SANs) from a certificate signing request (CSR) instead of the SANs that are included in the data of the certificate
  }))
